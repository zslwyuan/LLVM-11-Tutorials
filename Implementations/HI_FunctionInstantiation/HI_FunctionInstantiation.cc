...
bool HI_FunctionInstantiation::runOnModule(Module &M) // The runOnModule declaration will overide the virtual one in
...
char HI_FunctionInstantiation::ID = 0; // the ID for pass should be initialized but the value does not matter, since
            ...
            for (auto &I : B)
            {
                if (CallInst *callI = dyn_cast<CallInst>(&I))
                {
                    ...
                    if (ShouldFunctionBeInitiated(targetF))
                    {
                        ...
                        *FuncInitiationLog << "          ---- going to replace called function for CallInst [" << *callI
                                           << "] with function: [" << F_clone->getName() << "]\n";
...
void HI_FunctionInstantiation::pushCalledFunctionIntoQueue(Function *curF, std::queue<Function *> &FuncQ,
                                                           std::set<Function *> &FuncVisited)
{
    ...
    for (auto &B : *curF)
    {
        for (auto &I : B)
        {
            if (CallInst *callI = dyn_cast<CallInst>(&I))
            {
                ...
                {
                    *FuncInitiationLog << "          ---- push the sub-function [" << targetF->getName()
                                       << "] into queue\n";
                    FuncQ.push(targetF);
                }
            }
...
{
    *FuncInitiationLog << "          ---- replacing called function for CallInst [" << *callI << "] with function: ["
                       << F->getName() << "]\n";
