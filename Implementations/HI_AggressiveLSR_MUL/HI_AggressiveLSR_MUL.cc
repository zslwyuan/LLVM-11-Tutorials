...
char HI_AggressiveLSR_MUL::ID = 0; // the ID for pass should be initialized but the value does not matter, since LLVM
...
bool HI_AggressiveLSR_MUL::LSR_Mul(Instruction *I, ScalarEvolution *SE)
{
    ...
    */
    ...
    if (SARE)
    {
        ...
        {
            if (DEBUG)
                *AggrLSRLog << *I << " --> is add rec Mul: " << *SARE << " it operand (0) " << *SARE->getOperand(0)
            ...
            if (const SCEVConstant *start_V = dyn_cast<SCEVConstant>(SARE->getOperand(0)))
            {
                if (const SCEVConstant *step_V = dyn_cast<SCEVConstant>(SARE->getOperand(1)))
                {
                    ...
                    if (DEBUG)
                        *AggrLSRLog << *I
                                    << " --> is being replaced. start_val_APInt=" << start_val_APInt.getSExtValue()
...
bool HI_AggressiveLSR_MUL::LSR_Add(Instruction *I, ScalarEvolution *SE)
{
    ...
    */
    ...
    if (SARE)
    {
        ...
        {
            if (DEBUG)
                *AggrLSRLog << *I << " --> is add rec Quadratic Add: " << *SARE << " it operand (0) "
                            << *SARE->getOperand(0) << " it operand (1) " << *SARE->getOperand(1) << " it operand (2) "
                            << *SARE->getOperand(2) << "\n";
            if (const SCEVConstant *start_V = dyn_cast<SCEVConstant>(SARE->getOperand(0)))
            {
                if (const SCEVConstant *step_V = dyn_cast<SCEVConstant>(SARE->getOperand(1)))
